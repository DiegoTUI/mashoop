...
//checks if the api key provided is valid for the reuqested service
function validApiKey (apiKey, service, callback) {
	if (!config.checkApiKey) {
		return callback(null, true);
	}
	var db = new mongo.Db("mashoop", new mongo.Server("127.0.0.1", mongo.Connection.DEFAULT_PORT, {}), {w:1});
	async.waterfall ([
		//Open db
		function (callback) {
			db.open(callback);
		...
		function (db, callback) {
			db.collection('apikeys', callback);
		},
		//findOne
		function(collection, callback){
			collection.findOne({key: apiKey}, callback);
		}],
