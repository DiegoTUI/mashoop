...
exports.MSVenues = function(queryParameters) {
	...
	function makeRequest(callback) {
		...
		db.addCallback(function(error, result) {
			...
			var fields = {'name.ENG':1, imageList:1, 'descriptionList.ENG':1, availableModalityList:1, loc:1, code:1, indoor:1, morning:1, afternoon:1, evening:1, fullday:1};
			var options = {};
			// go find tickets
	 ...
	 */
	 function produceMobileResponse(data, callback) {
	 	...
	 	data.atlas.forEach(function(atlasElement) {
	 		...
	 		// price
	 		var priceList = atlasElement.availableModalityList[0].priceList;
	 		var adultPrice = priceList.filter(function(element) {
	 			return (element.description === "ADULT PRICE");
	 		});
	 		elementToAdd['price'] = adultPrice[0].amount;
	 		// add element
