...
exports.ATTicketAvail = function(queryParameters, descriptionMap, tag, forceCall) {
	...
	// default request
	var defaultRequest = atlasDefaults.ticketAvailRequest;
	...
	function getMemcachedKey(parameters) {
		return 'ticket#' + parameters.destination + '#' + parameters.language + '#' + parameters.from + '#' + parameters.to;
	}
	...
	function makeRequest(callback) {
		log.debug('Making TicketAvail ATLAS request...');
...
function testTicketAvailRequest (callback) {
	function atlasCallback(error, result) {
		...
		testing.assertEquals(result.ServiceTicketList.length, parseInt(result.TotalItems, 10), 'Wrong number of items retrieved', callback);
		testing.assertEquals(result.EchoToken, parameters.echoToken, 'Wrong echo token returned', callback);
		result.ServiceTicketList.forEach(function(ticket) {
			testing.assert(typeof ticket.AvailToken === 'string', 'ticket did not have availToken in it', callback);
			testing.assert(ticket.AvailToken.length > 0, 'ticket returned empty AvailTicket', callback);
		});
		testing.success(callback);
	}
	var parameters = {
		echoToken: 'testEchoToken',
		language: 'ENG',
		adults: '1',
		debug: true
	};
