...
/**
 * Find out if the argument is an integer.
 */
exports.isInteger = function (argument) {
  return argument===+argument && argument===(argument|0);
};

/**
 * Find out if the argument is a float.
 */
exports.isFloat = function (argument) {
  return argument===+argument && argument!==(argument|0);
};
...
exports.cloneObject = function(object) {
	...
	for (var i in object)
	{
		if (object[i] && typeof object[i] == 'object') {
			cloned[i] = exports.cloneObject(object[i]);
		}
		else
		{
			cloned[i] = object[i];
	...
	if (key === '#list') {  //It's a 'no-key' list
		...
		if (typeof value === 'string') {  //'regular' value
			...
			for (var j=0; j< value.length; j++) {
				for (var innerKeyList in value[j]) {
					result += processNode (innerKeyList, value[j][innerKeyList]);
				}
			}
			...
			for (var k=0; k<innerKeys.length; k++) {
				if (innerKeys[k].startsWith('@'))
					attributes.push(innerKeys[k].substringFrom('@'));
				else if (innerKeys[k] === '#value')
					textValue = value[innerKeys[k]];
				else
					other.push(innerKeys[k]);
			}
			//check the attributes first
			for (var l=0; l<attributes.length; l++) {
				result += ' ' + attributes[l] + '=\'' + value['@'+attributes[l]] + '\'';
			}
			//close the tag
			result += '>\n';
		...
		for (var m=0; m<other.length; m++) {
			result += processNode(other[m], value[other[m]]);
		}
		}
		//close the tag
		result += '</' + key + '>\n';
...
function testDecapitalizeKeys (callback) {
	...
	var result = exports.decapitalizeKeys(dictionary, exceptions);
...
function testXmlToJson (callback) {
	...
	var xmlString = exports.jsonToXml(jsonObject);
	for (var key in jsonObject) {
