	 		...
	 		if (typeof typeMapElement === 'object') {	//an object in the array
	 			...
	 			for (var i=0; i<jsonArray.length; i++) {
	 				if (typeof jsonArray[i] === 'string') {
	 					jsonArray[i] = getTypedValue (typeMapElement, jsonArray[i]);
	 				}
	 			}
	 		}
	 	});
...
function testFormatJsonObject (callback) {
	...
	for (var i=0; i<returnedJson["numbers"].length; i++) {
		...
		testing.assert(number === parseInt(jsonObject["numbers"][i]), "all numbers in array numbers should be the same as in the original object", callback)
	};
	...
	testing.assert(returnedJson['level1']['oneInt'] === parseInt(jsonObject['level1']['oneInt']), "level1.oneInt should be the same as in the original object", callback);
	...
	for (var i=0; i<returnedLevel2.length; i++) {
		testing.assert(core.isInteger(returnedLevel2[i]["oneInt"]), "level2-oneInt should be an integer", callback);
		testing.assert(core.isFloat(returnedLevel2[i]["oneFloat"]), "level2-oneFloat should be a float", callback);
		testing.assert(core.isString(returnedLevel2[i]["oneString"]), "level2-oneString should be a string", callback);
		testing.assert(returnedLevel2[i]["oneInt"] === level2[i]["oneInt"], "level2-oneInt should be the same in both objects",callback);
	}
