	...
	csvArray.forEach(function(csvItem) {
		...
		async.waterfall([
			...
			function (retrievedItem, callback) {
				...
				log.debug ("Found item: " + retrievedItem.code + ".Lat: " + parseFloat(csvItem[latitudeIndex]) + ". Lon: " + parseFloat(csvItem[longitudeIndex]))
				var setItem = {
			...
			function (error/*, result*/) {
				...
				processedItems++;
				log.debug("processed items: " + processedItems);
				// check if finished
